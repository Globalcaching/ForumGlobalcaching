<%@ WebHandler Language="C#" Class="InvisibleDog1.Imfuscator.Handler" %>
...
namespace InvisibleDog1.Imfuscator
{
    public class Handler : System.Web.IHttpHandler
    {
        public void ProcessRequest(System.Web.HttpContext context)
        {
            if (context.Request.QueryString["i"] != null)
            {
                ...
                string fn = Path.Combine(path, string.Format("{0}.png", value));
                if (File.Exists(fn))
                {
                ...
                fn = Path.Combine(path, string.Format("{0}.css", "style"));
                if (File.Exists(fn))
                {
                    context.Response.ContentType = GetContentType(fn);
                    byte[] buffer = File.ReadAllBytes(fn);
		...
		/*
            ...
            else
            {
                ...
                streamerType = assembly.GetType("InvisibleDog1.Imfuscator.Streamer");
